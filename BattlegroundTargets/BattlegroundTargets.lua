...
--local SetMapToCurrentZone = SetMapToCurrentZone -- 8.0 removed?
...
if isClassic then
	...
	LCD:Register("BattlegroundTargets") -- tell library it's being used and should start working
	UnitAura = LCD.UnitAuraWithBuffs
	LCD.RegisterCallback("BattlegroundTargets", "UNIT_BUFF", function(event, unit)
		BattlegroundTargets:UNIT_AURA(event, unit)
	end)
end
...
local pvptrinketIDs = {
	[GetSpellInfo(5579)]	= 300,	-- Immune Root/Snare/Stun 		Warrior, Hunter, Shaman
	[GetSpellInfo(23273)]	= 300,	-- Immune Charm/Fear/Polymorph	Rogue, Warlock
	[GetSpellInfo(23274)]	= 300, 	-- Immune Fear/Polymorph/Snare	Mage
	[GetSpellInfo(23276)]	= 300,	-- Immune Fear/Polymorph/Stun	Paladin, Priest
	[GetSpellInfo(23227)]	= 300,	-- Immune Charm/Fear/Stun		Druid
}
...
local function BuildBattlegroundMapTable()
	for i = 1, GetNumBattlegroundTypes() do
		...
		elseif bgID == 894 then mapID[1186]= localizedName bgMaps[localizedName] = {bgSize = 10, flagBG = 0} -- Seething Shore
		elseif bgID ==1014 then mapID[443] = localizedName bgMaps[localizedName] = {bgSize = 10, flagBG = 1} -- Warsong Gulch
		elseif bgID ==1018 then mapID[461] = localizedName bgMaps[localizedName] = {bgSize = 15, flagBG = 0} -- Arathi Basin 1366
			--	elseif bgID == then mapID[] = localizedName bgMaps[localizedName] = {bgSize = 40, flagBG = 0} -- 겨울 손아귀
		end
	end
end
...
local classROLES = {DAMAGER = {}}
for classID = 1, MAX_CLASSES + 2 do -- druid 11
	local classInfo = C_CreatureInfo.GetClassInfo(classID) -- 8.0 change
	local classTag
	...
	if classInfo then
		...
		classes[classTag].spec[1] = {role = 3, specID = nil, specName = classTag, icon = nil} tinsert(classROLES.DAMAGER, {classTag = classTag, specIndex = 1}) -- DAMAGER: total = 23
	end
	...
	--[[
	for i = 1, numTabs do
		local id, name, _, icon, role, _ = GetSpecializationInfoForClassID(classID, i)
...
local Textures = {}
Textures.AddonIcon     = "Interface\\AddOns\\BattlegroundTargets\\Media\\BattlegroundTargets-texture-button"
Textures.Path          = "Interface\\AddOns\\BattlegroundTargets\\Media\\BattlegroundTargets-texture-icons"
...
function BattlegroundTargets:CreateInterfaceOptions()
	...
	GVAR.InterfaceOptions.name = "BattlegroundTargets Classic"

	GVAR.InterfaceOptions.Title = GVAR.InterfaceOptions:CreateFontString(nil, "ARTWORK", "GameFontNormalLarge")
	GVAR.InterfaceOptions.Title:SetText("BattlegroundTargets Classic")
	...
	for frc = 1, #FRAMES do
		...
		for buttontype = 1, 2 do
			for i = 1, 40 do
				...
				button.PVPTrinketTexture:SetTexture("Interface\\Icons\\Inv_jewelry_trinketpvp_01")				--old   ("Interface\\Icons\\INV_Jewelry_TrinketPVP_01")
				--button.PVPTrinketTexture:SetTexture( 214027)				--old   ("Interface\\Icons\\INV_Jewelry_TrinketPVP_01")
...
function BattlegroundTargets:CreateOptionsFrame()
	...
	GVAR.OptionsFrame.Title:SetText("BattlegroundTargets Classic")
	...
	GVAR.OptionsFrame.TestShuffler.Texture:SetTexture("Interface\\Common\\friendship-heart")
...
function BattlegroundTargets:DefaultShuffle()
	for frc = 1, #FRAMES do
		...
		for i = 1, 40 do
			...
			local classToken = class_IntegerSort[ 1 ].cid
			local talentSpec = classes[ classToken ].spec[ 1 ].role
	...
	if isConfig then
		if SummaryToggle then --TODO summary option split results
			for frc = 1, #FRAMES do
				...
				Summary.DamageEnemy:SetText(DATA.Enemy.Roles[1])   -- DAMAGER ENEMY
			end
		end
...
function BattlegroundTargets:BattlefieldScoreUpdate()
	...
	for index = 1, numScore do
		...
		local name, _, _, _, _, faction, _, _, _, classToken, _, _, _, _, _, talentSpec = GetBattlefieldScore(index) -- TODO leming
...
--[[		if classes[classToken].fix then
			if classes[classToken].fixname[talentSpec] ~= nil then
...
function BattlegroundTargets:IsBattleground()
	...
	local bgName, zoneName, mapName, size, fb
	...
	if not currentBGMap then
		...
		for i = 1, GetMaxBattlefieldID() do -- GetMaxBattlefieldID() = 1
			...
			if queueMapName == AV then
				size = 40
				fb = 0
			elseif queueMapName == WG then
				size = 10
				fb = 1
			elseif queueMapName == AB then
				size = 15
				fb = 0
			else
				size = 10
	...
	if DATA.PvPTrinketEndTime[unitName] then
		...
		if trinketTime > 0 then
			...
			if not isLowLevel then
				if id == 195901 then
					button.PVPTrinketTexture:SetTexture("Interface\\Icons\\Sha_ability_rogue_sturdyrecuperate")
				else
					--button.PVPTrinketTexture:SetTexture( GetSpellInfo(3, select(1, GetSpellInfo(id))) ) -- TODO trinket tex
					button.PVPTrinketTexture:SetTexture( "Interface\\Icons\\Inv_jewelry_trinketpvp_02" )
				end
			end
...
local function CombatLogPVPTrinketCheck(clEvent, spellId, sourceName) -- pvp_trinket_
	if pvptrinketIDs[spellId] and (clEvent == "SPELL_CAST_SUCCESS" or clEvent == "SPELL_AURA_APPLIED") then
...
function BattlegroundTargets:CheckPlayerLevel() -- LVLCHK
	...
	elseif playerLevel < maxLevel then
		--isLowLevel = true
		isLowLevel = nil -- LVLCHK error? leming
